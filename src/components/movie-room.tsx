...
import { COMMENT_MAX, NICKNAME_MAX, posterUrl, runtimeLabel, statusLabels, type Feed, type Movie, type MovieStatus, type TmdbResult } from "@/lib/model";
...
export default function MovieRoom() {
  ...
  <main className="shell"><section className="intro"><div><p className="eyebrow">COMMUNITY WATCHLIST</p><h1>“はると”が次に観る映画を、みんなで。</h1><p className="intro-copy">おすすめを投稿して、気になる一本に「おすすめしたい」を。ひとこと添えると、もっと伝わります。</p></div><button className="primary" onClick={openPost}><span aria-hidden="true">＋</span> 映画をおすすめする</button></section>
  ...
  <div className="movie-body"><div className="movie-title"><h3><Link href={`/movies/${m.id}`}>{m.title}</Link></h3>{m.status !== "unwatched" && <span className={`badge ${m.status}`}>{statusLabels[m.status]}</span>}</div><p className="meta">{m.release_year ? `${m.release_year}年公開` : "公開年未登録"}{m.runtime ? <><span>·</span>{runtimeLabel(m.runtime)}</> : null}<span className="posted-at">·</span><span className="posted-at">{`${new Date(m.created_at).toLocaleDateString("ja-JP", { month: "short", day: "numeric", timeZone: "Asia/Tokyo" })} に届いたおすすめ`}</span></p>
  ...
  <button className={`vote ${m.voted ? "selected" : ""}`} aria-pressed={m.voted} aria-label={`${m.title}：${m.vote_count}票。${m.voted ? "おすすめを取り消す" : "おすすめしたいに投票してひとこと書く"}`} disabled={!!pendingVote || m.status === "watched"} onClick={() => m.voted ? void cancelVote(m) : (setFormError(""), setVoteFor(m))}><span className="vote-number"><span aria-hidden="true">{m.voted ? "♥" : "♡"}</span> {m.vote_count}</span><span>{pendingVote === m.id ? "更新中…" : m.status === "watched" ? "受付終了" : m.voted ? "おすすめ済み" : "おすすめしたい"}</span></button></article>; })}</div>}
  ...
  <aside className="sidebar"><section className="guide-card"><p className="eyebrow"><span aria-hidden="true">✦</span> HOW TO</p><h2>おすすめの届け方</h2><ol><li><span>01</span><div><strong>まずは映画名で検索</strong><p>同じ作品がもうあれば、そこに「おすすめしたい」を。</p></div></li><li><span>02</span><div><strong>なければ、新しく投稿</strong><p>候補から選ぶだけ。あなたの1票も一緒に入ります。</p></div></li><li><span>03</span><div><strong>ひとこと添える</strong><p>どこが good か、ネタバレなしで。作品ページに並びます。</p></div></li></ol><div className="ticket-foot">TELL HARUTO WHAT TO WATCH</div></section>
  {/* 観る予定が決まっていないときに空の枠だけ出ても伝わらないので、入っているときだけ見せる。 */}
  {feed?.next.length ? <section className="next-card"><p className="eyebrow"><span aria-hidden="true">✦</span> UP NEXT</p><h2>はるとが次に観る</h2><p className="aside-copy">みんなのおすすめから、観る予定に入れた作品です。</p><ol className="next-list">{feed.next.map(m => <li key={m.id}><Link href={`/movies/${m.id}`}><strong>{m.title}</strong><span>{m.release_year || "公開年未登録"} <span aria-hidden="true">↗</span></span></Link></li>)}</ol><div className="ticket-foot">SAVE A SEAT FOR THE NEXT ONE</div></section> : null}</aside></div>
  ...
  {modal === "post" && <Modal title={picked ? "おすすめポイントを書く" : "どの映画をおすすめしますか？"} onClose={closeModal} busy={busy}>
    {!picked ? <>
      ...
      <p className="form-note">投稿時にあなたの「おすすめしたい」が1票入ります。</p>
    </form>}
  </Modal>}
